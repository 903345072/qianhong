<template>
	<view class="zaiui-my-box" :class="show?'show':''">
		<view class="bg-gradual-red zaiui-head-box" style="background-image:linear-gradient(45deg, #6bb4f9, #4e90f7) ;">
			...
			<!--用户信息-->
			<view v-if="user != null" class="content text-xl" style="left: 0;background: url(/static/img/backgroundCard.png) no-repeat 0 0/100% 100%;width: 92%;margin-left: 4%;padding: 35rpx 20rpx;">
				<view style="display: flex;">
					<view style="position: relative;margin-right: 40rpx;">
						<img class="cu-avatar round lg" :src="user.avatar" />
				...
				<view style="padding-top: 30rpx;display: flex;justify-content: space-between;">
					<text style="color: grey;font-size:27rpx;">{{user.self_intro?user.self_intro:"还未介绍自己"}}</text>
					<text @tap="editIntro(user.uid)" class="cuIcon-writefill" style="color: grey;"></text>
				</view>
			</view>
			...
			editIntro(uid){
				...
				uni.showModal({
					...
					success(res) {
						if(res.confirm){
							if(res.content){
								that.$api.setUserIntro({"uid":uid,"content":res.content}).then(res=>{
								
									uni.showToast({
										title:'修改成功',
										success() {
											that.user.self_intro =res.data
										}
									})
								})
							}
