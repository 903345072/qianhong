	...
	export default {
		...
		onLoad(option) {
		    ...
		    this.$api.getUserInfo().then(res=>{
				...
				that.myuid = res.data.data.uid+"uid"
				that.uname = res.data.data.real_name
				that.shop_id = res.data.data.shop_id+"shopid"
			
				that.uavatar = res.data.data.avatar
		...
		methods:{
			...
			connectSocketInit() {
			                ...
			                this.socketTask = uni.connectSocket({
			                    ...
			                    url: "ws://8.130.121.124:82?id="+this.myuid,
			                    success(data) {
			...
			screenMsg(msg){
				...
				//存储到本地
				let msga = uni.getStorageSync(this.shop_id+'msglist'+this.myuid);
				if(!msga){
					var msg_ = []
					msg_.push(msg)
					uni.setStorageSync(this.shop_id+'msglist'+this.myuid,msg_);
				}else{
					msga.push(msg)
					uni.setStorageSync(this.shop_id+'msglist'+this.myuid,msga);
				}
			...
			loadHistory(e){
				...
				setTimeout(()=>{
					...
					let list_ = []
					var ll = uni.getStorageSync(this.shop_id+'msglist'+this.myuid)
					if(ll){
						list_ = ll.reverse()
					}
					let list =  (offset + this.pageSize >= list_.length) ? list_.slice(offset, list_.length) : list_.slice(offset, offset + this.pageSize).reverse();
			...
			getMsgList(){
			    ...
			    this.msgList =  (offset + this.pageSize >= list.length) ? list.slice(offset, list.length).reverse() : list.slice(offset, offset + this.pageSize).reverse();
						...
						for(let i=0;i<res.tempFilePaths.length;i++){
							uni.getImageInfo({
								...
								success: (image)=>{
									...
									that.uploadPhoto(res.tempFilePaths[i],image)
			...
			uploadPhoto(tempFilePaths,image) {
				var that = this
				uni.uploadFile({
					url: config.config.url+"/api/user/uploadChatPhoto", //开发者服务器 url
					header:{
					...
					success(res) {
						...
						var img_url = JSON.parse(res.data).data	
						let msg = {url:img_url,w:image.width,h:image.height};
			...
			sendMsg(content,type){
				...
				let lastid = this.msgList.length>0? this.msgList[this.msgList.length-1].msg.id:0;
				lastid++;
				let msg = {u_type:'user',toId:this.shop_id,type:'user',msg:{id:lastid,time:nowDate.getHours()+":"+nowDate.getMinutes(),type:type,userinfo:{uid:this.myuid,username:this.uname,face:this.uavatar},content:content}}
				var that = this
