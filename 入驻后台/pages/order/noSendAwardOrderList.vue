<template>
	<view class="zaiui-home-box" :class="show?'show':''">
		...
		<view  style="display: flex;align-items: center;justify-content: center;background-color: #b46a6b;width: 70%;margin-left: 15%;position: fixed;margin-top: 10rpx;">
			<view style="width: 50%;text-align: center;padding: 10rpx 0;" :style="curIndex==1?'background:yellow;color:red;':'background:white;color:grey'" @click="changeIndex(1)">普通订单</view>
			<view  @click="changeIndex(2)" style="width: 50%;text-align: center;;padding: 10rpx 0" :style="curIndex==2?'background:yellow;color:red;':'background:white;color:grey'">跟单订单</view>
		</view>
		...
		<view class="bg-white zaiui-footer-fixed zaiui-foot-padding-bottom" >
			<view class="cu-bar padding-lr">
				<view class="checked-view" @tap="tapChecked"> 
					...
					<text style="margin-left: 10rpx;color: #007AFF;font-size: 24rpx;" v-if="curIndex==2">预计跟单扣费{{getflow_fee()}}元</text>
				</view>
				<view class="btn-view" style="display: flex;align-items: center;">
					<view style="font-size: 30rpx;font-weight: bold;">合计:{{getTotal()}}</view>
	...
	export default {
		...
		methods: {
			...
			getList(flag){
				...
				var data = {"state":this.state,"pageNo":this.pageNo,"pageSize":this.pageSize,"type":this.game_type_arr[this.game_index]["en_name"],"flow_index":this.curIndex}
			...
			sendAward(){
				...
				this.can_dian = true
				var data = {"ids":[],"type":this.curIndex,"flow_yj":this.getflow_fee()}
			...
			getOrderDetail(item){
				var that = this
				that.$api.getOrderDetail({"id":item.id}).then(res=>{
						this.orderdetail = item
					that.orderdetail["dd"] = res.data.dd
					that.orderdetail["order_detail_"] = res.data.order_detail_
