<template>
	<view v-if="user != null">
		...
		<view  @click="transferUsers()" style="border-bottom: 6px solid #EEEEEE;">
			<view style="display: flex;justify-content: center;align-items: center;margin-top: 20rpx;margin-bottom: 20rpx;">
				<view style="font-size: 36rpx;" >
					转移客户
				</view>
		...
		<view v-if="user != null" style="width: 100%;padding: 20rpx 20rpx;line-height: 80rpx;display: flex;flex-direction: column;justify-content: center;">
			...
			<view style="display: flex;justify-content: space-between;border-bottom: 1px solid #EEEEEE;padding: 15rpx 0;">
				<text style="color: #000000;">是否启用</text>
				<switch @change="setStatus(user.uid)" :checked="user.status == 1" color="#007AFF"></switch>
			</view>
			<!-- <view style="display: flex;justify-content: space-between;border-bottom: 1px solid #EEEEEE;padding: 15rpx 0;">
	...
	export default{
		...
		methods:{
			...
			transferUsers(){
				...
				uni.showModal({
					...
					success(res) {
						if(res.confirm){
							if(res.content){
								that.$api.transferUser({"uid":that.user.uid,"real_name":res.content}).then(res=>{
								
									uni.showToast({
										title:'修改成功',
										success() {
											that.getData({"uid":that.user.uid})
										}
									})
								})
							}
			...
			setStatus(uid){
				...
				var data = {"uid":uid,"value":this.status}
				this.$api.setStatus(data).then(res=>{
					uni.showToast({
						title:"操作成功"
					})
				})
