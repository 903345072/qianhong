<template>
	<view>
		<bar-title bgColor="bg-white" :isBack="true" >
			...
			<block slot="content">{{u_name}}</block>
		</bar-title>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			this.uid = option.uid
			this.u_name = option.u_name
			var that = this
		    this.$api.getShopInfo().then(res=>{
					
				that.myuid = res.data.uid+"shopid"
				that.uname = res.data.name

				that.uavatar = res.data.logo
		...
		methods:{
			                    ...
			                    this.socketTask.onMessage((res) => {
			                     ...
			                     var msgs = JSON.parse(res.data)
								  ...
								  if(msgs["msg"]["type"] != "ping" && msgs["msg"]["type"] != "bind"){
									if(msgs["msg"]["userinfo"]["uid"]==this.uid+"uid")
									   	that.screenMsg(msgs);
								  }
			...
			screenMsg(msg){
				...
				//存储到本地
				let msga = uni.getStorageSync('msglist'+this.myuid);
				if(!msga){
					var msg_ = []
					msg_.push(msg)
					uni.setStorageSync('msglist'+this.myuid,msg_);
				}else{
					msga.push(msg)
					uni.setStorageSync('msglist'+this.myuid,msga);
				}
			...
			loadHistory(e){
				...
				setTimeout(()=>{
					...
					let list = []
					this.$api.getChatList({"uid":this.uid,"pageNo":this.pageNo}).then(res=>{
						list = res.data
						// 获取消息中的图片,并处理显示尺寸
			...
			getMsgList(){
				...
				this.$api.getChatList(p).then(res=>{
			...
			uploadPhoto(tempFilePaths,image) {
				var that = this
				uni.uploadFile({
					url: config.config.baseUrl+"User/uploadChatPhoto", //开发者服务器 url
					header:{
			...
			sendMsg(content,type){
				...
				lastid++;
				var uuid = this.uid+"uid"
				let msg = {u_type:'shop',toId:uuid,type:'user',msg:{id:lastid,time:nowDate.getHours()+":"+nowDate.getMinutes(),type:type,userinfo:{uid:this.myuid,username:this.uname,face:this.uavatar},content:content}}
				var that = this
